...
void shapesBegin()
{
    ...
    const GLchar* textFragmentShaderCode =
        ...
        "void main()\n"
        "{\n"
        "   color = vec4(1.0, 1.0, 1.0, texture(bitmap, texPos));\n"
        "}\n";
...
void shapesGenerateText(float upper_x, float lower_x, float upper_y, float lower_y,
                        float layer, char* bitmap, int bitmap_width, int bitmap_height,
                        unsigned int* TID, unsigned int *VAO, unsigned int *VBO)
{
    float vertices[] =
    {
        upper_x, upper_y, layer, 1.0, 0.0,
        upper_x, lower_y, layer, 1.0, 1.0,
        lower_x, lower_y, layer, 0.0, 1.0,
        lower_x, upper_y, layer, 0.0, 0.0,
    };
